function load_story_map(filename) {
  var documentSettings = {};

  // Some constants, such as default settings
  const CHAPTER_ZOOM = 15;

  // TODO !! CONGFIGURATION SHOULD BE IN ROOT CONFIG.YAML !!

  // First, try reading Options.csv
  $.get('story-maps/0-configuration.csv', function(options) {
	// If found INITIALISE THE MAP
    $.get('story-maps/'+ filename +'.csv', function(chapters) {
      initMap(
        $.csv.toObjects(options),
        $.csv.toObjects(chapters)
      )
    }).fail(function(e) { alert('Found 0-configuration.csv, but could not read Chapters.csv') });

  // Nested functions !?
  }).fail(function(e) { alert('Could not read 0-configuration.csv or Chapters.csv') });
  ...
  */
  ...
  function initMap(options, chapters) {
    ...
    /* This is done through Jekyll
	document.title = getSetting('_mapTitle');
    $('#map-header').append('<h1>' + (getSetting('_mapTitle') || '') + '</h1>');
    $('#map-header').append('<h2>' + (getSetting('_mapSubtitle') || '') + '</h2>');
    ...
    if (getSetting('_mapLogo')) {
      ...
      $('#map-header').css('padding-top', '25px');
    }
	*/
    // Load tiles
    addBaseMap();
    ...
    for (i in chapters) {
      ...
      // Create an empty chapter container (div )
	  // error in the original (<div></div> : jQuery dialects ???
      var container = $('<div> ', {
        id: 'container' + i,
    ...
    $('div#map-contents').scroll(function() {
      var currentPosition = $(this).scrollTop();
	
		
      // Make title disappear on scroll 
     if (currentPosition < 200) {
       $('#map-title').css('opacity', 1 - Math.min(1, currentPosition / 100));
      }
    ...
    $('#map, #narration, #map-title').css('visibility', 'visible');
    $('div.loader').css('visibility', 'hidden');

    $('div#container0').addClass("in-focus");
    $('div#map-contents').animate({scrollTop: '1px'});
